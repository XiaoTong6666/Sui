...
public class SuiService extends Service<SuiUserServiceManager, SuiClientManager, SuiConfigManager> {
            ...
            if (autoRestart) {
                for (String packageName : packageNames) {
                    try {
                        ...
                        LOGGER.w(e, "Failed to restart package %s", packageName);
                    }
                }
            }
        } finally {
    ...
    private void invalidateFallbacks(
            @NonNull java.util.Collection<ClientFallback> fallbacks, boolean autoRestart, String reason) {
        ...
        for (java.util.Map.Entry<Integer, java.util.Set<String>> entry : packagesByUid.entrySet()) {
            invalidatePackages(entry.getKey(), entry.getValue(), false, reason);
        }
            ...
            if (autoRestart) {
                for (java.util.Map.Entry<Integer, java.util.Set<String>> entry : packagesByUid.entrySet()) {
                    for (String packageName : entry.getValue()) {
                        try {
                            AppLaunchUtils.startAppAsUser(packageName, UserHandleCompat.getUserId(entry.getKey()));
                        } catch (Throwable e) {
                            LOGGER.w(e, "Failed to restart fallback package %s", packageName);
                        }
                    }
                }
            }
        ...
        for (java.util.Map.Entry<Integer, java.util.Set<String>> entry : affectedPackagesByUid.entrySet()) {
            ...
            if (requiresRootCapabilityReset(oldDefaultMode, newDefaultMode)) {
                invalidatePackages(
                        uid,
                        entry.getValue(),
                        newDefaultMode == SuiConfig.FLAG_ALLOWED_SHELL,
                        "Root permission revoked");
                ...
                if (!fallbacks.isEmpty()) {
                    invalidateFallbacks(fallbacks, true, "Binder handoff failed");
                }
            } else {
                invalidatePackages(uid, entry.getValue(), false, "Default permission changed");
            }
        }
    }
        ...
        if (!onetime) {
            ...
            try {
                ...
                if (!shellMode && requiresRootCapabilityReset(oldPermissionFlags, permissionFlags)) {
                    invalidatePackagesForUid(
                            requestUid, permissionFlags == SuiConfig.FLAG_ALLOWED_SHELL, "Root permission revoked");
                } else {
                ...
                if (!shellFallbacks.isEmpty()) {
                    invalidateFallbacks(shellFallbacks, true, "Shell client migration failed");
                }
                if (!handoffFallbacks.isEmpty()) {
                    invalidateFallbacks(handoffFallbacks, true, "Binder handoff failed");
                }
            } finally {
                if (capabilityBarrier) {
        ...
        try {
            ...
            if (newEffectiveFlags != oldEffectiveFlags) {
                if (requiresRootCapabilityReset(oldEffectiveFlags, newEffectiveFlags)) {
                    invalidatePackagesForUid(
                            uid, newEffectiveFlags == SuiConfig.FLAG_ALLOWED_SHELL, "Root permission revoked");
            ...
            if (!shellFallbacks.isEmpty()) {
                invalidateFallbacks(
                        shellFallbacks,
                        getServerUidForPermissionFlags(newEffectiveFlags) != -1,
                        "Shell client migration failed");
            }
        } finally {
            if (capabilityBarrier) {
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
                ...
                try {
                    ...
                    if (!shellMode) {
                        ...
                        if (!shellFallbacks.isEmpty()) {
                            invalidateFallbacks(
                                    shellFallbacks,
                                    getServerUidForPermissionFlags(targetMode) != -1,
                                    "Shell client migration failed");
                        }
                    }
                } finally {
        ...
        if (code == ServerConstants.BINDER_TRANSACTION_registerUserServiceProcess) {
            ...
            boolean processGroup = data.readInt() != 0;
            try {
                int callingPid = Binder.getCallingPid();
                boolean registered = userServiceManager.registerUserServiceProcess(token, callingPid, processGroup);
                reply.writeNoException();
