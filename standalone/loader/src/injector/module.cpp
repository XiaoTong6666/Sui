...
void ZygiskContext::run_modules_pre() {
    LOGI("SuiLoader: run_modules_pre started."); // [日志]
    auto ms = zygiskd::ReadModules();
    LOGI("SuiLoader: Read %zu modules from daemon.", ms.size());
    auto size = ms.size();
    for (size_t i = 0; i < size; i++) {
        ...
        if (void *handle = DlopenMem(m.memfd, RTLD_NOW);
            ...
            LOGI("SuiLoader: Successfully dlopened libsui.so, handle=%p", handle); // [日志]
            modules.emplace_back(i, handle, entry);

            using sui_init_t = void(ApiTable *, JNIEnv *);
            auto sui_init_func = (sui_init_t*) dlsym(handle, "sui_standalone_init");

            if (sui_init_func) {
                LOGI("SuiLoader: Found sui_standalone_init at %p, calling it.", sui_init_func); // [日志]
                sui_init_func(modules.back().getApiTable(), env);
            } else {
                LOGE("SuiLoader: CRITICAL - sui_standalone_init not found!"); // [日志]
            }
        } else {
            LOGE("SuiLoader: CRITICAL - Failed to dlopen libsui.so from memfd!"); // [日志]
        }
    }

    // 调用原始的 onLoad 和 pre... 函数
    for (auto &m : modules) {
        m.onLoad(env);
